		...
		<view v-if="!isLoading" class="goods-info m-top20">
			...
			<view class="info-item info-item__top dis-flex flex-x-between flex-y-end">
				<view class="block-left dis-flex flex-y-center">
					<!-- 商品售价 -->
					<template v-if="goods.tuan_goods_price==0">
						<text class="floor-price__samll">￥</text>
						<text class="floor-price">{{ goods.goods_price_min }}</text>
					</template>
					<template v-else>
						<view class="tuan" v-if="goods.tuan_goods_price>0">团购价</view>
						<!-- 团购价钱 -->
						<text class="floor-price__samll">￥</text>
						<text class="floor-price">{{ goods.tuan_goods_price }}</text>
					</template>
					<!-- 会员价标签 -->
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			// 团购商品ID
			this.tuanGoodsId = parseInt(options.tuanGoodsId || 0)
			// 加载页面数据
			this.onRefreshPage()
		},
		...
		methods: {
			...
			getGoodsDetail() {
				...
				const param = {
					goodsId: app.goodsId,
					tuanGoodsId: app.tuanGoodsId
				}
				return new Promise((resolve, reject) => {
					GoodsApi.detail(param)
						.then(result => {
							app.goods = result.data.detail
							app.goods.tuanGoodsId = app.tuanGoodsId
							resolve(result)
						})
						.catch(reject)
				})
