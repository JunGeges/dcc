<template>
	<view class="home-conatiner">
		...
		<mescroll-body ref="mescrollRef" @init="mescrollInit" :down="{ native: true,use:true }" @down="downCallback"
			...
			<u-tabs :list="cateList" :is-scroll="true" active-color="#fa2209" :current="currentCateIndex"
				@change="getCateId">
			</u-tabs>

			<u-notice-bar v-if="noticeList.length>0" mode="horizontal" color="#de8c17" :list="noticeList">
			</u-notice-bar>


			<view class="shop-box">
				<view class="shop-item" v-for="item,index in list.data" :key="index"
					@click="toShopDetail(item.shop_id)">
	...
	export default {
		...
		methods: {
			...
			toShopDetail(store_shop_id) {
				this.$navTo('pages/merchants/home', {
					store_shop_id
				})
			},
